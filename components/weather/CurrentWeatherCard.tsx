...
import { Droplets, Wind, Eye, Gauge, Sun, Cloud } from 'lucide-react';
...
const StatCard: React.FC<{ icon: React.ReactNode; label: string; value: string; sub?: string; color: string }> = ({ icon, label, value, sub, color }) => (
  <div className="flex items-center gap-3 p-3 rounded-xl bg-white/60 backdrop-blur-sm border border-white/40">
...
const CurrentWeatherCard: React.FC<CurrentWeatherProps> = ({ data, locationName }) => {
  ...
  return (
    <motion.div
      ...
      className="bg-gradient-to-br from-brand-800 via-brand-900 to-brand-900 rounded-2xl p-6 text-white shadow-xl overflow-hidden relative"
    >
      ...
      <div className="relative z-10">
        <div className="flex items-start justify-between mb-4">
          <div>
            <p className="text-brand-100 text-sm font-medium">{locationName}</p>
        ...
        <div className="flex items-end gap-4 mb-6">
          <span className="text-7xl sm:text-8xl font-extrabold tracking-tight leading-none">{data.temperature}°</span>
          <div className="pb-2">
            <p className="text-brand-100 text-sm">Feels like {data.apparentTemperature}°C</p>
            <p className="text-brand-200 text-xs">H: {data.temperature + 3}° L: {data.temperature - 4}°</p>
          </div>
        </div>
