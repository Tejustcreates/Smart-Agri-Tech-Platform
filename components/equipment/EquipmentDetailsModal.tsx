...
const EquipmentDetailsModal: React.FC<EquipmentDetailsModalProps> = ({ listing, onClose }) => {
  ...
  const imgs = listing ? (listing.images.length ? listing.images : [listing.image]) : [];
      ...
      {listing && (
        <>
          ...
          <div className="relative bg-gray-100 overflow-hidden">
            ...
            <button onClick={onClose} aria-label="Close" className="tap-target absolute top-3 right-3 w-12 h-12 bg-white/80 rounded-full flex items-center justify-center shadow hover:bg-white">
              <X size={18} />
            </button>
            {listing.verified && (
