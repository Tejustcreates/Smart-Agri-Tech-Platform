      ...
      <div className="w-full max-w-7xl mx-auto px-6 lg:px-12 py-16">
        ...
        {!modelReady && (
          <div className="mb-6 p-4 bg-brand-50 rounded-xl flex items-center gap-3 max-w-xl mx-auto">
            ...
            <span className="text-brand-700 text-sm font-medium">Training prediction model...</span>
          </div>
        )}

        {/* Tabs — big tappable buttons */}
        <div className="flex justify-center mb-8">
          <div className="bg-gray-100 rounded-2xl p-1.5 flex gap-1.5 w-full max-w-md">
            {tabs.map((tab) => (
              <button
                key={tab.key}
                onClick={() => setActiveTab(tab.key)}
                className={`flex-1 py-3.5 rounded-xl text-sm font-bold transition-all flex items-center justify-center gap-2 tap-target ${
                  activeTab === tab.key
            ...
            {result ? (
              ...
              <div className="bg-white rounded-2xl shadow-sm p-12 text-center border border-gray-100 flex-1 flex flex-col items-center justify-center">
                ...
                <p className="text-gray-500 mb-6">
                  {activeTab === 'manual'
                    ? 'Enter your soil data on the left and tap "Get Crop Recommendation"'
                    : 'Connect your ESP32 device or tap "Run Prediction" with simulated data'}
                </p>
                <div className="flex flex-wrap justify-center gap-2">
                  {['Rice', 'Wheat', 'Cotton', 'Sugarcane', 'Potato'].map((crop) => (
                    <span key={crop} className="px-3 py-1.5 bg-brand-50 text-brand-700 rounded-full text-xs font-medium">{crop}</span>
                  ))}
                </div>
              </div>
            )}
