...
import {
  ...
  Grid2x2, Landmark as BuildingColumns, Info, Phone, PhoneCall, ExternalLink, MessageCircle, Video, Send,
} from 'lucide-react';
import { ROUTES } from '../constants';

const FOOTER_LINKS = [
  { name: 'Live Mandi Prices', route: ROUTES.MANDI, icon: Store },
  { name: 'Weather Forecast', route: ROUTES.WEATHER, icon: CloudSun },
  { name: 'Crop Doctor', route: ROUTES.DISEASE, icon: Bug },
  { name: 'Crop Advisor', route: ROUTES.CROPS, icon: Sprout },
  { name: 'Govt Schemes', route: ROUTES.SCHEMES, icon: Landmark },
  { name: 'Equipment Rental', route: ROUTES.EQUIPMENT, icon: Tractor },
  { name: 'Farmer News', route: ROUTES.NEWS, icon: Newspaper },
];

const LEGAL_LINKS = [
  { name: 'Privacy Policy', slug: 'privacy' },
  { name: 'Terms of Use', slug: 'terms' },
  { name: 'Farmer Guidelines', slug: 'guidelines' },
];

const SOCIAL_LINKS = [
  { icon: MessageCircle, label: 'WhatsApp Agri Community' },
  { icon: Video, label: 'YouTube Farming Guides' },
  { icon: Send, label: 'Telegram Weather Alerts' },
];
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-16 relative z-10">
        ...
        <div className="pb-12 border-b border-white/10 grid grid-cols-1 lg:grid-cols-12 gap-8 items-center">
          ...
          <div className="lg:col-span-5 bg-white/5 border border-white/15 rounded-2xl p-5 backdrop-blur-sm">
            ...
            >
              <Phone size={13} /> Call 1800-180-1551 (Toll-Free)
            </a>
          </div>
        </div>
        ...
        <div className="py-12 grid grid-cols-1 md:grid-cols-3 gap-10 border-b border-white/10">
          ...
          <div>
            <h3 className="text-white font-extrabold text-sm uppercase tracking-wider mb-5 flex items-center gap-2">
              <Grid2x2 size={13} className="text-emerald-400" />
              Platform Tools
            </h3>
            <ul className="space-y-1">
              {FOOTER_LINKS.map((link) => (
                <li key={link.name}>
                  ...
                  >
                    <link.icon size={13} className="text-emerald-400 flex-shrink-0" />
                    <span>{link.name}</span>
                  </button>
                </li>
              ))}
          ...
          <div>
            <h3 className="text-white font-extrabold text-sm uppercase tracking-wider mb-5 flex items-center gap-2">
              <BuildingColumns size={13} className="text-emerald-400" />
              ...
              {GOVT_PORTALS.map((portal) => (
                <li key={portal.name}>
                  ...
                  >
                    ...
                    <ExternalLink size={11} className="opacity-40 group-hover:opacity-100 transition-opacity flex-shrink-0" />
                  </a>
                </li>
              ))}
            </ul>
          ...
          <div>
            ...
            <div className="flex gap-2">
              {SOCIAL_LINKS.map((s) => (
                <Tooltip key={s.label} title={`${s.label} — coming soon`}>
                  <span
                    aria-disabled="true"
                    aria-label={`${s.label} (coming soon)`}
                    className="w-9 h-9 bg-white/5 rounded-xl flex items-center justify-center text-emerald-200/40 cursor-not-allowed"
                  >
                    <s.icon size={15} />
                  </span>
                </Tooltip>
        ...
        <div className="py-6 flex flex-col sm:flex-row items-center justify-between gap-4 text-xs text-emerald-100/50">
          ...
          <div className="flex gap-4">
            {LEGAL_LINKS.map((link) => (
              <Link key={link.slug} to={`/legal/${link.slug}`} className="hover:text-emerald-200 transition-colors">
                {link.name}
              </Link>
            ))}
          </div>
