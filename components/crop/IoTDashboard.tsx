...
const IoTDashboard: React.FC<IoTDashboardProps> = ({ sensorData, connected, battery, wifi, lastSync, onConnect, onDisconnect, onSync, onRunPrediction, predicting }) => {
  const sensors = [
    { label: 'Nitrogen', value: sensorData.nitrogen.toFixed(1), unit: 'kg/ha', icon: <Leaf size={16} className="text-brand-600" />, color: 'bg-brand-50', status: getStatus(sensorData.nitrogen, 20, 80), trend: sensorData.nitrogen > 50 ? 'up' as const : 'down' as const },
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="bg-white rounded-2xl border border-gray-100 p-5 shadow-sm">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            <div className={`w-3 h-3 rounded-full ${connected ? 'bg-brand-500 animate-pulse' : 'bg-gray-300'}`} />
            <div>
              <p className="text-sm font-bold text-gray-800">ESP32 Sensor Hub</p>
              <p className={`text-xs font-medium ${connected ? 'text-brand-600' : 'text-gray-400'}`}>
      ...
      <div className="flex flex-wrap gap-3">
        {!connected ? (
          <button onClick={onConnect} className="px-5 py-2.5 bg-brand-600 text-white rounded-xl text-sm font-semibold hover:bg-brand-800 transition-all shadow-sm active:scale-95 tap-target">
            Connect Device
          </button>
        ) : (
          <button onClick={onDisconnect} className="px-5 py-2.5 bg-gray-200 text-gray-700 rounded-xl text-sm font-semibold hover:bg-gray-300 transition-all active:scale-95 tap-target">
            Disconnect
          </button>
        )}
        <button onClick={onSync} disabled={!connected} className="px-5 py-2.5 bg-brand-400 text-white rounded-xl text-sm font-semibold hover:bg-brand-600 disabled:bg-gray-200 disabled:text-gray-400 transition-all shadow-sm active:scale-95 tap-target">
          Sync Data
        </button>
        <button onClick={onRunPrediction} className="px-5 py-2.5 bg-brand-600 text-white rounded-xl text-sm font-semibold hover:bg-brand-800 transition-all shadow-sm active:scale-95 tap-target">
